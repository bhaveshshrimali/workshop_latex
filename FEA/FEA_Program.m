...
NodeTable = [0        0
    L        0
    L+d(1,storej) H+d(2,storej)
    d(3,storej) H+d(4,storej)];
...
for n=1:tSteps-1
    ...
    d(:,storej) = dn(:,n) + delt*vn(:,n) + delt^2/2*(1-2*beta)*an(:,n);
    v(:,storej) = vn(:,n) + delt*(1-gamma)*an(:,n);
    acc(:,storej) = an(:,n);
    ...
    while (res>tol*Res_0) && iter < maxiter
        ...
        NodeTable = [0        0
                     L        0
                     L+d(1,storej+1) H+d(2,storej+1)
                     d(3,storej+1) H+d(4,storej+1)];
...
%     end
    ...
    dn(:,n) = d(:,storej);    vn(:,n) = v(:,storej);    an(:,n) = acc(:,storej);
